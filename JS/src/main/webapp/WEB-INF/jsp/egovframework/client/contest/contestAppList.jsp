...
$(document).ready(function () {
	...
	pagingUtil.__url			= "<c:url value='/contest/contestAppList.do'/>";
...
function goBoardView(val){
	...
	$("#frmDetail").attr("action", "<c:url value='/contest/contestAppView.do'/>");
	$("#ct_seq").val(val);
	$("#frmDetail").submit();
}
...
function goLaneView(val,flag){
	if(val != 0){ //(flag =='E' || flag =='F') && 
		if( val == '231'){
			...
			$("#frmLane").attr("action","<c:url value='/contest/contestLaneResult.do'/>");
			$("#frmLane").submit();
		}
	}else{
...
function goAppResult(val,flag){
	if((flag =='E' || flag =='F') && val != 0){
		if( val == '231'){
			...
			$("#frmDetail").attr("action", "<c:url value='/contest/contestAppResult.do'/>");
			$("#ct_seq").val(val);
			$("#frmDetail").submit();
		}
	}else{
