...
function goBoardUpdt() {
	$("#frm").attr("action", "<c:url value='/board/shopEventModify.do'/>");
	$("#frm").submit();
}

function goBoardDel() {
	if(!confirm('삭제하시겠습니까?')) return;
	$("#frm").attr("action", "<c:url value='/board/shopEventDelete.do'/>");
	$("#frm").submit();
}
...
function goPreBoardView(no) {
	$("#frm").attr("action", "<c:url value='/board/shopEventDetail.do'/>");
	$("#ntt_id").val(no);
	$("#frm").submit();
}
					...
					<div class="board_view_area sub_fade">
						<div class="bbs_view">
							<div class="view_head">
								...
								<dl class="file">
									<dt class="t_tit">이름 : </dt>
									<dd class="info">
										<span><c:out value='${boardView.ntcr_nm }' /></span>
									</dd>
								</dl>
								<dl class="file">
									<dt class="t_tit">연락처 : </dt>
									<dd class="info">
										<span><c:out value='${boardView.as_tel }' /></span>
									</dd>
								</dl>
								<dl class="file">
									<dt class="t_tit">이메일주소 : </dt>
									<dd class="info">
										<span><c:out value='${boardView.as_email }' /></span>
									</dd>
								</dl>
								<dl class="file">
									<dt class="t_tit">소속 볼링장 : </dt>
									<dd class="info">
										<span><c:out value='${boardView.temp1 }' /></span>
									</dd>
								</dl>
								<dl class="file">
									<dt class="t_tit">보유 정비기계 : </dt>
									<dd class="info">
										<span><c:out value='${boardView.temp2 }' /></span>
									</dd>
								</dl>
								<dl class="file">
									<dt class="t_tit">참석여부 : </dt>
									<dd class="info">
										<span><c:out value='' />
											<c:if test="${boardView.temp3 eq '1'}">1부 참석</c:if>
											<c:if test="${boardView.temp3 eq '2'}">2부 참석</c:if>
											<c:if test="${boardView.temp3 eq '3'}">1부, 2부 참석</c:if>
										</span>
									</dd>
								</dl>
							</div>
