...
function goUpdt() {
	...
	$("#frm").attr("action", "<c:url value='/board/consultUpdt.do'/>");
	$("#frm").submit();
}
								...
								<table summary="게시판 글쓰기 표로 제목, 구분, 내용,첨부파일 등이 있습니다.">
									...
									<tbody>
										...
										<tr>
											...
											<td><textarea id="ntt_cn" name="ntt_cn">${boardView.ntt_cn }</textarea></td>
										</tr>
										<tr>
											...
											<td> <!-- class="fileBtn" -->
						                		...
						                		<c:choose>
													<c:when test="${fn:length(consultFile) > 0}">
														<c:forEach items="${consultFile}" var="asFile" varStatus="status">
