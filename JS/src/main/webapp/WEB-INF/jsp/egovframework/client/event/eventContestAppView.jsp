...
function goApp(){
	$("#frm").attr("action","/event/eventContestAppWrite.do");
	$("#frm").submit();
}
...
function chkApp(){
	...
	if(isLogin == false){
		...
		$("#returnUrl").val("eventContestAppWrite");
		...
		<div id="contents" class="s_con">
	        ...
	        <div class="sub_content">
	            <div class="inner">
		            <div class="sub_head">
						<c:choose>
							<c:when test="${contestView.ct_type eq 'S' }">
			        ...
			        <div class="board_view_area sub_fade">
			            <div class="bbs_view">
			                <div class="view_head">
								...
								<c:if test="${contestView.situation_show_yn eq 'Y' && contestView.ct_type ne 'S' }">
				                    <dl class="file">
				                       	<table>
					                       	<tbody>
					                       		<tr>
													...
													<c:choose>
														...
														<c:when test="${contestView.ct_type eq 'L' }">
															<c:choose>
																<c:when test="${fn:length(partAppList) > 0 }">
																	<c:forEach items="${partAppList }" var="list">
																		<td><span style="font-weight: bold; ">${list.part_ord }조 ${list.part_name }</span> : <span style="color: blue; font-weight: bold;"><c:out value='${list.app_cnt }'/>/<c:out value='${contestView.limit_part + contestView.limit_waiting  }'/></span></td>
																	</c:forEach>
																</c:when>
															</c:choose>
														</c:when>
