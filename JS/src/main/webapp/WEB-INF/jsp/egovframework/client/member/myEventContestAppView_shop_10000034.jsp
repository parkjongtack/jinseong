...
function goAppInfoUpdt(){
	...
	if(!$("input[name=option1]").is(":checked")){
		alert("1번 문제의 정답을 선택해주세요.");
		$("input[name=option1]").focus();
		return;
	} else {
		if($("input[name=option1]:checked").val() != "프로톤 피직스"){
			alert("1번 문제의 정답이 틀렸습니다.\n다시 생각해보세요.");
			$("input[name=option1]").focus();
			return;
		}
	}
                                    ...
                                    <tbody>
                                    	<tr>
	                                        <th scope="col" colspan="4">
                                   	  	...
                                   	  	<tr>
	                                        <td colspan="4">
	                                        	<p class="opt_title">1. 진승의 25주년을 기념하기 위해 기념 로고가 각인된 볼의 이름은? (힌트 ㅍㄹㅌ ㅍㅈㅅ)</p>	
	                                        	<p class="opt_wrap">
                              			            <label><input type="radio" name="option1" value="솔라 패럴랙스" <c:if test="${contestView.option1 eq '솔라 패럴랙스' }">checked="checked"</c:if>>솔라 패럴랙스</label>
	                                        		<label><input type="radio" name="option1" value="컴플리트 피직스" <c:if test="${contestView.option1 eq '컴플리트 피직스' }">checked="checked"</c:if>>컴플리트 피직스</label>
	                                        		<label><input type="radio" name="option1" value="프로톤 피직스" <c:if test="${contestView.option1 eq '프로톤 피직스' }">checked="checked"</c:if>>프로톤 피직스</label>
	                                        	</p>
	                                        </td>
	                                    </tr>
                                   	  		<tr>
	                                        <td colspan="4">
	                                        	<p class="opt_title">2. STORM, ROTO GRIP, 900GLOBAL볼 중 나의 인생볼은?(주관식, 10자이내)</p>	
	                                        	<p class="opt_wrap">
	                                        		<input type="text" id="option2" name="option2" maxlength="10" value="<c:out value='${contestView.option2 }'/>">
	                                        	</p>
	                                        </td>
	                                    </tr>                            
                                        <tr>
