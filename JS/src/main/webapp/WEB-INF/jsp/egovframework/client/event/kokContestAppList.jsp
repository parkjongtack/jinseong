...
$(document).ready(function () {
	...
	pagingUtil.__url			= "<c:url value='/event/kokContestAppList.do'/>";
...
function goBoardView(val){
	...
	$("#frmDetail").attr("action", "<c:url value='/event/kokContestAppView.do'/>");
	$("#ct_seq").val(val);
	$("#frmDetail").submit();
}
...
function goLaneView(val,flag){
	if(val != 0){ //(flag =='E' || flag =='F') && 
		if( val == '231'){
			...
			$("#frmLane").attr("action","<c:url value='/event/kokContestLaneResult.do'/>");
			$("#frmLane").submit();
		}
	}else{
...
function goAppResult(val,flag){
	if((flag =='E' || flag =='F') && val != 0){
		if( val == '231'){
			...
			$("#frmDetail").attr("action", "<c:url value='/event/kokContestAppResult.do'/>");
			$("#ct_seq").val(val);
			$("#frmDetail").submit();
		}
	}else{
