...
function goBoardUpdate(){
	...
	$("#frm").attr("action", "<c:url value='/apage/event/kokContestMngUpdt.do'/>");
	$("#frm").submit();
}

function fileDelete(no){
	if(!confirm('삭제하시겠습니까?')) return;
	$("#frm").attr("action", "<c:url value='/apage/event/kokContestMngFileDel.do'/>");
				...
				<table>
					...
					<tbody>
						...
						<tr>
							<th>조별정원</th>
							<td class="al_l" colspan="3">
								<input type="number" id="limit_part" name="limit_part" title="조별정원 입력" class="w7p" value="${contestView.limit_part }" > 명
								<input type="hidden" id="limit_waiting" name="limit_waiting" title="대기자정원 입력" class="w60p" value="${contestView.limit_waiting }" >
							</td>
						</tr>
						<tr>
							<th>노출여부</th>
						...
						<tr>
							...
							<th>자동배치할 레인 수</th>
							<td class="al_l">
								<input type="number" name="lane_num" value="${contestView.lane_num}" >
