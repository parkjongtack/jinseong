...
function goBoardUpdate(){
	...
	var check_yn = $('input:radio[name="check_yn"]:checked').val();
	if(check_yn == undefined || check_yn == null || check_yn == ""){
	...
	$("#frm").attr("action", "<c:url value='/apage/board/adminNoticeUpdate.do'/>");
	$("#frm").submit();
}

function fileDelete(no){
	if(!confirm('삭제하시겠습니까?')) return;
	$("#frm").attr("action", "<c:url value='/apage/board/NoticeFileDel.do'/>");
				...
				<table>
					...
					<tbody>
						...
						<tr>
							<th>상단공지여부</th>
							<td class="al_l">
								<input type="radio" id="close" name="check_yn" value="Y" <c:if test="${boardView.check_yn eq 'Y' }">checked</c:if> /><label for="close">Y(사용)</label>
								<input type="radio" id="open" name="check_yn" value="N" <c:if test="${boardView.check_yn eq 'N' }">checked</c:if> /><label for="open">N(사용 안함)</label>
							</td>
							<th>사용여부</th>
							<td class="al_l">
