...
function goBoardList(){
	$("#frm").attr("action", "<c:url value='/apage/contest/adminContestList.do'/>");
	$("#frm").submit();
}
...
function goBoardUpdate(){
	...
	$("#frm").attr("action", "<c:url value='/apage/contest/adminContestUpdate.do'/>");
	$("#frm").submit();
}

function fileDelete(no){
	if(!confirm('삭제하시겠습니까?')) return;
	$("#frm").attr("action", "<c:url value='/apage/contest/ContestFileDel.do'/>");
				...
				<table>
					...
					<tbody>
						<tr>
							<th>제목</th>
							<td class="al_l">
								<input type="text" id="ntt_sj" name="ntt_sj" title="제목 입력" class="w40p" maxlength="150" value="<c:out value='${contestView.ntt_sj }'/>" >
							</td>
							<th>작성자</th>
							<td class="al_l">
								<c:out value='${contestView.ntcr_id }'/>
							</td>
						</tr>
						<tr>
							<th>상단공지여부</th>
							<td class="al_l">
								<input type="radio" id="close" name="check_yn" value="Y" <c:if test="${contestView.check_yn eq 'Y' }">checked</c:if>  /><label for="close">사용</label>
								<input type="radio" id="open" name="check_yn" value="N" <c:if test="${contestView.check_yn eq 'N' }">checked</c:if> /><label for="open">사용안함</label>
							</td>
							<th>사용여부</th>
							<td class="al_l">
								<input type="radio" id="close" name="use_at" value="Y" <c:if test="${contestView.use_at eq 'Y' }">checked</c:if>  /><label for="close">사용</label>
								<input type="radio" id="open" name="use_at" value="N" <c:if test="${contestView.use_at eq 'N' }">checked</c:if> /><label for="open">사용안함</label>
							</td>
						</tr>
												
						<tr class="content">
							<td class="cont al_l" colspan="4">
								<textarea id="ntt_cn" name="ntt_cn" style="width:100%;height:320px;" title="내용 입력"><c:out value='${contestView.ntt_cn }'/></textarea>
							</td>
						</tr>
						<tr>
							<th>첨부파일</th>
