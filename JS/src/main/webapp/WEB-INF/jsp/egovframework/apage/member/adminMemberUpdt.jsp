...
/* 	if($("#pwCheckPre").val() != "Y" || $("#pwCheck").val() != "Y"){
	...
	$("#frm").attr("action", "<c:url value='/apage/member/adminMemberUpdate.do'/>");
	$("#frm").submit();
}
        	...
        	<div class="bbs-write">
				<table>
					...
					<tr>
						<th>성별</th>
						<td class="al_l">
							<label><input type="radio" name="mber_gender" id="mber_gender1" value="M" class="type-radio required" title="성별 남 선택" <c:if test="${memberVo.mber_gender eq 'M' }">checked</c:if> /> 남</label>
                            <label><input type="radio" name="mber_gender" id="mber_gender2" value="F" class="type-radio required" title="성별 여 선택" <c:if test="${memberVo.mber_gender eq 'F' }">checked</c:if> /> 여</label>						
						</td>
					</tr>
					<tr>
...
<c:if test="${!empty msg }" >
	...
	if(msg != ''){
		if(msg.indexOf('success') > 0){
			alert('수정되었습니다.');	
			location.href = '/apage/member/adminMemberList.do';
		}
