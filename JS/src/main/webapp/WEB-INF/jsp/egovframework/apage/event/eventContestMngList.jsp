...
$(document).ready(function () {
	...
	pagingUtil.__url			= "<c:url value='/apage/event/eventContestMngList.do'/>";
		...
		if(confirm("자리배치 하시겠습니까?") == true){
			...
			$.ajax({
				...
				success		: function(data) {
					...
					$("#frmDetail").attr("action","/apage/event/eventContestInsertAutoLane.do");
					$("#frmDetail").submit();
...
function goBoardView(val){
	...
	$("#frmDetail").attr("action", "<c:url value='/apage/event/eventContestMngDetail.do'/>");
	$("#ct_seq").val(val);
	$("#frmDetail").submit();
}
...
function goReg(){
	location.href = "<c:url value='/apage/event/eventContestMngWrite.do'/>";
}
...
<form:form commandName="vo" id="frmDetail" name="frmDetail">
	...
	<div id="container">
		...
		<div class="contents">
			...
			<!-- //ct-tab -->
			<ul class="ct-tab">
				<li><a href="/apage/event/eventContestList.do" class="" id="part_firBtn">대회안내</a></li>
				<li><a href="/apage/event/eventContestMngList.do" class="on">대회신청관리</a></li>
		            		...
		            		<c:when test="${fn:length(contestList) > 0}">
		            			<c:forEach items="${contestList }" var="list" varStatus="status">
		            				<tr>
			                            ...
			                            <td class="al_c">
			                            	<c:if test="${list.ct_type eq 'A'}"><span>프로암 이벤트</span></c:if>
			                            	<c:if test="${list.ct_type eq 'L'}"><span>프리미엄 레슨</span></c:if>
			                            	<c:if test="${list.ct_type eq 'S'}"><span>이벤트</span></c:if>
	                            		</td>
			                            <td class="al_c"><c:if test="${list.ct_process eq 'E' && list.ct_type ne 'S'}"> <a href="javascript:void(0);" data-seq="${list.ct_seq}" data-prepareYn="${list.prepare_yn }"class="btnAutoLane s-blue-btn">자리배치</a></c:if></td>
			                            ...
			                            <td><c:out value='${list.part1_app_cnt }'/>/<c:out value='${list.limit_part * list.part_cnt}'/></td>
