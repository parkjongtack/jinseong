...
$(document).ready(function () {
	...
	pagingUtil.__url			= "<c:url value='/apage/board/adminStaffList.do'/>";
...
function goBoardView(val){
	...
	$("#frmDetail").attr("action", "<c:url value='/apage/board/adminStaffModify.do'/>");
	$("#seq").val(val);
	$("#frmDetail").submit();
}
...
function goReg(){
	location.href = "<c:url value='/apage/board/adminStaffWrite.do'/>";
}
	...
	<div id="container">
		...
		<div class="contents">
        	<ul class="ct-tab">
                ...
                <li class="tab1"><a href="<c:url value='/apage/board/adminStaffList.do'/>" class="on">STAFF관리</a></li>
						...
						<c:choose>
		            		<c:when test="${fn:length(staffList) > 0}">
		            			<c:forEach items="${staffList }" var="list" varStatus="status">
		            				<tr onclick="goBoardView('${list.seq}');" style='cursor:pointer;'>
										<th scope="row"><c:out value='${(totalNum+1)-list.ascnum }'/></th>
										<td>
			                            	<c:if test="${list.atch_file_id ne null && list.atch_file_id ne '' }">
			                            		<img src="/commonfile/nunFileDown.do?atch_file_id=${list.atch_file_id }" alt="대표이미지" width="100px"/>
			                            	</c:if>
			                            ...
			                            <td><c:out value='${list.asso_type }'/></td>
			                            <td><c:out value='${list.staff_type }'/></td>					                            
