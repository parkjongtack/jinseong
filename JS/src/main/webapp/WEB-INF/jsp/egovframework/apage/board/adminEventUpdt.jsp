...
function goBoardList(){
	$("#frm").attr("action", "<c:url value='/apage/board/adminEventList.do'/>");
	$("#frm").submit();
}
...
function goBoardUpdate(){
	...
	$("#frm").attr("action", "<c:url value='/apage/board/adminEventUpdate.do'/>");
	$("#frm").submit();
}

function fileDelete(no){
	if(!confirm('삭제하시겠습니까?')) return;
	$("#frm").attr("action", "<c:url value='/apage/board/EventFileDel.do'/>");
					...
					<tbody>
						<tr>
							...
							<td class="al_l" colspan="3">
								<input type="text" id="ntt_sj" name="ntt_sj" title="제목 입력" class="w40p" maxlength="150" value="<c:out value='${boardView.ntt_sj }'/>" >
							</td>
						</tr>
						<tr>
							<th>사용여부</th>
						...
						<tr>
							...
							<td class="al_l" colspan="4">
		                		...
		                		<c:choose>
									<c:when test="${fn:length(eventFile) > 0}">
										<c:forEach items="${eventFile}" var="list" varStatus="status">
										<div>
