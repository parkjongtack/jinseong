...
<head>
	...
	<body>
		<table border="1">
			<tr>
				<td colspan="8" style="text-align:center; font-weight:bold;font-size: 22px;">${contestVo.ct_sbj }</td>				
			</tr>
			<tr>
				<td colspan="8" style="text-align:right;">출력일자 : <%=strDate %></td>				
			</tr>
		</table>
  			...
  			<c:when test="${fn:length(resultList) > 0 }">
  				<c:forEach items="${resultList }" var="fList" varStatus="status">
  					<table style="border:1px solid #c9c9c9;width:45%">
  						<tbody>
		     				<tr>
		     					<td colspan="7" style="text-align:center;color:red">${status.count }조 선정자 명단</td>
		     				</tr>
     						<tr>
								...
								<td style="text-align:center; background-color:#c9c9c9">파운드</td>
  			...
  			<c:when test="${fn:length(readyResultList) > 0 }">
  				<c:forEach items="${readyResultList }" var="fList" varStatus="status">
  					<table style="border:1px solid #c9c9c9;width:45%">
  						<tbody>
		     				<tr>
		     					<td colspan="8" style="text-align:center;color:red">${status.count }조 대기자 명단</td>
		     				</tr>
     						<tr>
 								<td style="text-align:center; background-color:#c9c9c9">No</td>
								<td style="text-align:center; background-color:#c9c9c9">성명</td>
