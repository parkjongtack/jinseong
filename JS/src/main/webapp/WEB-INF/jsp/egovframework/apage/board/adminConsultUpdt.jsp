...
function goBoardUpdt(){
	...
	$("#frm").attr("action", "<c:url value='/apage/board/adminConsultUpdate.do'/>");
	$("#frm").submit();
}

function goBoardDel(){
	if(!confirm('삭제하시겠습니까?')) return;
	$("#frm").attr("action", "<c:url value='/apage/board/adminConsultDelete.do'/>");
	$("#frm").submit();
}

function fileDelete(no){
	if(!confirm('삭제하시겠습니까?')) return;
	$("#frm").attr("action", "<c:url value='/apage/board/ConsultFileDel.do'/>");
					...
					<tbody>
						...
						<tr class="content">
							<td class="cont al_l" colspan="4">
								<textarea id="ntt_cn" name="ntt_cn" style="width:100%;height:320px;" title="내용 입력"><c:out value='${boardView.ntt_cn }' escapeXml="false"/></textarea>			
							</td>
						</tr>
					</tbody>
				</table>
