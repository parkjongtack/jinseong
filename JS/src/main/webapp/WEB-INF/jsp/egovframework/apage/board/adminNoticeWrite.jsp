...
function goBoardList(){
	$("#frm").attr("action", "<c:url value='/apage/board/adminNoticeList.do'/>");
	$("#frm").submit();
}
...
function goBoardWrite(){
	...
	var auth_code_chk = $('input:radio[name="check_yn"]:checked').val();
	if(auth_code_chk == undefined || auth_code_chk == null || auth_code_chk == ""){
		alert("상단공지여부를 체크해주세요.");
		return;
	}
	
	var auth_code_chk2 = $('input:radio[name="use_at"]:checked').val();
	if(auth_code_chk2 == undefined || auth_code_chk2 == null || auth_code_chk2 == ""){
		alert("사용여부를 체크해주세요.");
		return;
	}
	...
	$("#frm").attr("action", "<c:url value='/apage/board/adminNoticeReg.do'/>");
	$("#frm").submit();
}
					...
					<tbody>
						...
						<tr>
							...
							<td class="al_l">
								...
								<input type="radio" id="open" name="check_yn" value="N" checked="checked" /><label for="open">N(사용 안함)</label>
							</td>
							<th>사용여부</th>
							<td class="al_l">
								<input type="radio" id="close2" name="use_at" value="Y" checked="checked" /><label for="close2">Y(사용)</label>
								<input type="radio" id="open2" name="use_at" value="N"  /><label for="open2">N(사용 안함)</label>
							</td>
						</tr>
						<tr class="content">
