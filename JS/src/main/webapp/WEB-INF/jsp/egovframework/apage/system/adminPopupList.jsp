...
$(document).ready(function () {
	...
	pagingUtil.__url			= "<c:url value='/apage/system/adminPopupList.do'/>";
...
function goBoardView(val){
	...
	$("#frmDetail").attr("action", "<c:url value='/apage/system/adminPopupDetail.do'/>");
	$("#pop_seq").val(val);
	$("#frmDetail").submit();
}
...
function goReg(){
	location.href = "<c:url value='/apage/system/adminPopupWrite.do'/>";
}
	...
	<div id="container">
		...
		<div class="contents">
        	<ul class="ct-tab">
                ...
                <li class="tab1"><a href="<c:url value='/apage/system/adminPopupList.do'/>" class="on">팝업관리</a></li>
			...
			<fieldset class="bbs-list">
				<legend>팝업관리 목록</legend>
				<table summary="게시판관리 전체 목록">
					<colgroup><col width="7%" /><col width="*" /><col width="*" /><col width="7%" /><col width="10%" /></colgroup>
					<thead>
						<tr>
							<th scope="col">번호</th>
							<th scope="col">제목</th>
						...
						<c:choose>
		            		<c:when test="${fn:length(popupList) > 0}">
		            			<c:forEach items="${popupList }" var="list" varStatus="status">
		            				<tr onclick="goBoardView('${list.pop_seq}');" style='cursor:pointer;'>
										<th scope="row"><c:out value='${(totalNum+1)-list.ascnum }'/></th>
			                            <td><c:out value='${list.pop_subject }'/></td>
			                            <td><c:out value='${list.pop_st_dt }'/>~<c:out value='${list.pop_ed_dt }'/></td>
										<td>
											<c:if test="${list.pop_state eq 'Y' }">게시</c:if>
											<c:if test="${list.pop_state eq 'N' }">종료</c:if>
										</td>
										<td><c:out value='${list.pop_reg_dt }'/></td>
									</tr>
				                </c:forEach>
				            </c:when>
				            <c:otherwise>
