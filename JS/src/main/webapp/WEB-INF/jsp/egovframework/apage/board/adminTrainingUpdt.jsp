...
function goBoardWrite(){
	...
	$("#frm").attr("action", "<c:url value='/apage/board/adminTrainingUpdt.do'/>");
	$("#frm").submit();
}

function goBoardDel(){
	if(!confirm('삭제하시겠습니까?')) return;
	$("#frm").attr("action", "<c:url value='/apage/board/adminTrainingListDelete.do'/>");
	$("#frm").submit();
}

function fileDelete(no){
	if(!confirm('삭제하시겠습니까?')) return;
	$("#frm").attr("action", "<c:url value='/apage/board/adminTrainingListFileDel.do'/>");
				...
				<table>
					...
					<tbody>
						...
						<tr>
							<th>사용자페이지 URL</th>
							<td class="al_l" colspan="3">
								<span>https://bowlingkorea.com/kegel/trainingView.do?ntt_id=<c:out value='${laneView.ntt_id }'/>&currRow=1</span>
							</td>
						</tr>
						<tr class="content">
							<th>제품설명</th>	
