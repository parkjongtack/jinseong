...
<%
    ...
    response.setHeader("Content-Disposition","attachment;filename="+URLEncoder.encode("왕중왕전 신청자_리스트_")+ strDate + ".xls");
...
<head>
	...
	<body>
		<table border="1">
			<tr>
				<td colspan="12" style="text-align:center; font-weight:bold;font-size: 22px;">${contestVo.ct_sbj }</td>				
			</tr>
			<tr>
				<td colspan="12" style="text-align:right;">출력일자 : <%=strDate %></td>				
			</tr>			
			<tr>
				<td colspan="12" style="text-align:center; font-weight:bold;font-size: 18px;">1조</td>				
			</tr>			
			<tr>
				...
				<td style="text-align:center; background-color:#c9c9c9">티셔츠<br/>사이즈</td>
            		...
            		<c:when test="${fn:length(part1List) > 0}">
            			<c:forEach items="${part1List }" var="list" varStatus="status">
            				<tr>
								...
								<td>
									<c:if test="${list.status eq '0001'}">입금대기</c:if>
									<c:if test="${list.status eq '0002'}">입금완료</c:if>
		...
		<table border="1">
			<tr>
				<td colspan="12" style="text-align:center;  font-weight:bold;font-size: 18px;">2조</td>				
			</tr>			
			<tr>
				<td style="text-align:center; background-color:#c9c9c9">No</td>
				<td style="text-align:center; background-color:#c9c9c9">참가자명</td>
