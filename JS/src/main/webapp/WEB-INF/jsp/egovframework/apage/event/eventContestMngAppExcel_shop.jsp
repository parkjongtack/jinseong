            		...
            		<c:when test="${fn:length(contestAppList) > 0}">
            			<c:forEach items="${contestAppList }" var="list" varStatus="status">
            				<tr>
								...
								<td>
										<c:if test="${list.option1 eq '1'}">DOMINATION</c:if>
										<c:if test="${list.option1 eq '2'}">LOCK</c:if>
										<c:if test="${list.option1 eq '3'}">CODE</c:if>
										<c:if test="${list.option1 eq '4'}">GATE</c:if>
										<c:if test="${list.option1 eq '5'}">PHYSIX</c:if>
									
									</td>
									<td class="colorOpt">
										<c:if test="${list.option2 eq '1'}">IQ</c:if>
										<c:if test="${list.option2 eq '2'}">MARVEL</c:if>
										<c:if test="${list.option2 eq '3'}">PHAZE</c:if>
										<c:if test="${list.option2 eq '4'}">AXIOM</c:if>
									</td>		
								<td>
