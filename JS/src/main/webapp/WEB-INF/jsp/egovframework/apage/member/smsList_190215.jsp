		...
		success		: function(data) {
			...
			if(data.root.resultCode == "Y"){
				...
				for(var i = 0; i < aList.length; i++){
					...
					//체크박스
					html += '<td class="al_c">';
					html += '<input type="checkbox" id="chkMber" class="chkMber" value="'+aList[i].mber_tel+'">';
					html += '</td>';
						...
						<c:choose>
							<c:when test="${fn:length(smsAgreeList) > 0}">
								<c:forEach items="${smsAgreeList }" var="list" varStatus="status">
									<tr>
										<td class="al_c"><input type="checkbox" id="chkMber" class="chkMber" value="${list.mber_tel }:${list.mber_name }"></td>
