...
function partAdd(){
	...
	html += '<input type="hidden" class="plOrd" name="list['+((pCnt)-1)+'].part_ord" value="'+((pCnt))+'">';
	html += '<input type="hidden" class="plSeq" name="list['+((pCnt)-1)+'].ecp_seq" value="0">';
	
	
	html += '<input type="text" class="w20p plName" name="list['+((pCnt)-1)+'].part_name" value="" style="margin-right: 4px;">';
...
function etcPartReOrdering(){
	$("#plConWrap p").each(function(idx){
		...
		$(this).find(".plName").attr("name","list["+idx+"].part_name");
		$(this).find(".plOrd").attr("name","list["+idx+"].part_ord");
		$(this).find(".plOrd").val((idx+1));
		$(this).find(".plSeq").attr("name","list["+idx+"].ecp_seq");
...
function goBoardUpdate(){
	...
	$("#frm").attr("action", "<c:url value='/apage/event/eventContestMngUpdt.do'/>");
	$("#frm").submit();
}

function fileDelete(no){
	if(!confirm('삭제하시겠습니까?')) return;
	$("#frm").attr("action", "<c:url value='/apage/event/eventContestMngFileDel.do'/>");
				...
				<table>
					...
					<tbody>
						...
						<tr>
							<th>대회타입</th>
							<td class="al_l" colspan="3">
								<select title="접수타입 선택" name="ct_type" id="ct_type"  class="w20p" onchange="ctTypeChange(this.value)">
                                	<option value="">::대회타입선택::</option>
                                	<option value="L" <c:if test="${contestView.ct_type eq 'L' }">selected="selected"</c:if>>프리미엄 레슨 이벤트</option>
                                	<option value="A" <c:if test="${contestView.ct_type eq 'A' }">selected="selected"</c:if>>프로암 이벤트</option>
                                	<option value="S" <c:if test="${contestView.ct_type eq 'S' }">selected="selected"</c:if>>이벤트</option>
					...
					<tbody id="ctTypeWrap" <c:if test="${contestView.ct_type ne 'L' }">style="display: none;"</c:if>>
						<c:if test="${fn:length(pList) > 0}">
							<tr>
								<th>프리미엄 레슨 이벤트</th>
								<td class="al_l" colspan="3" id="plConWrap">
									<c:forEach items="${pList }" var="pList">
										<p id="plWrap${pList.part_ord }" class="plCl" style="margin-bottom: 2px;">
											<span>${pList.part_ord }조</span>
											<input type="hidden" name="ecp_seq_arr" value="${pList.ecp_seq }">
											<input type="hidden" class="plOrd" name="list[${pList.part_ord - 1}].part_ord" value="${pList.part_ord }">
											<input type="hidden" class="plSeq" name="list[${pList.part_ord - 1}].ecp_seq" value="${pList.ecp_seq }">
											<input type="text" class="w20p plName" name="list[${pList.part_ord - 1}].part_name" value="${pList.part_name }" >
											<a href="javascript:void(0);" class="btnAdd s-blue-btn">추가</a>
											<a href="javascript:void(0);" class="btnDel s-gry-btn" data-seq="${pList.part_ord }">삭제</a>
										</p>
									</c:forEach>
								</td>
							</tr>
