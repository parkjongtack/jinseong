						...
						<c:choose>
							<c:when test="${fn:length(smsHistory) > 0}">
								<c:forEach items="${smsHistory }" var="list" varStatus="status">
									<fmt_rt:formatDate value="${list.reqdate}" pattern="yyyyMM" />
									<%-- <c:set var="sms_send_date_ym" value="<fmt_rt:formatDate value="${list.reqdate}" pattern="yyyyMM" />"/> --%>
									<tr onclick="goSmsView('${list.etc3}','${sms_send_date_ym }')" style="cursor: pointer;">
										<td>${status.count }</td>
										<td class="al_l">${list.msg }</td>
										<td>
											<c:if test="${list.etc4 eq 'endContestMember' }">대회발송</c:if>
											<c:if test="${list.etc4 eq 'allMember' }">전체발송</c:if>
											<c:if test="${list.etc4 eq 'selectMember' }">선택발송</c:if>
										</td>
										<td>${list.reqdate }</td>
									</tr>
								</c:forEach>
