...
function goModify(){
	$("#frm").attr("action", "<c:url value='/apage/board/adminNoticeModify.do'/>");
	$("#frm").submit();
}

function goDelete(){
	if(!confirm('삭제하시겠습니까?')) return;
	$("#frm").attr("action", "<c:url value='/apage/board/adminNoticeDelete.do'/>");
	$("#frm").submit();
}
			...
			<table>
				...
				<tbody>
					...
					<tr>
						<th>상단공지여부</th>
						<td class="al_l">
							<c:if test="${boardView.check_yn eq 'Y' }">사용</c:if>
							<c:if test="${boardView.check_yn eq 'N' }">사용안함</c:if>
						</td>
						<th>사용여부</th>
						<td class="al_l">
